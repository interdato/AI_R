...
subtitle: "De la predicción a la acción"
format:
  revealjs:
    slide-number: true
    transition: slide
    toc: false
    scrollable: true   # <- permite scroll vertical dentro de las slides
editor: visual
execute:
  echo: false
  warning: false
...
- Pregunta clave: **¿A quién contactar para maximizar el retorno?**
...
- Variables de comportamiento y atributos:
  ...
  - Segmento, región, precio del plan

---

## 🔧 Setup (librerías y semilla)
...
pkgs <- c("tidyverse","pROC","xgboost","scales","knitr","DT")
vers <- vapply(pkgs, function(p) as.character(utils::packageVersion(p)), character(1))
knitr::kable(tibble(Paquete = pkgs, Versión = vers), caption = "Paquetes cargados y versiones")
```

---

## 🧪 Datos sintéticos (B2B)
...
datos <- tibble(
  ...
  mutate(
    # Función oculta para simular probabilidad real de compra
    lp = -2.0 +
         0.20 * visitas_web_30 +
...
knitr::kable(head(datos, 6), caption = "Muestra de datos sintéticos")
```

---

## ✂️ Partición & matrices para XGBoost
...
# Render de tablas con DT y scroll
tagList(
  ...
  tags$h4("Columnas de la matriz de diseño (train)"),
  DT::datatable(
    primeras_cols,
    rownames = FALSE,
    options = list(
      dom = "t",
      paging = FALSE,
      scrollY = "320px",  # <- scroll vertical interno
      scrollX = TRUE,     # <- por si hay columnas extras en el futuro
      autoWidth = TRUE
    ),
...
## 🏋️ Entrenamiento del modelo (XGBoost)
...
knitr::kable(tibble(`Mejor iteración` = mejor_iter, `AUC (eval)` = round(auc_eval,4)),
...
ggplot(cal_df, aes(x = p_media, y = tasa_real)) +
  geom_point(size = 2) + geom_line() +
  geom_abline(linetype = 2) +
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title = "Calibración (deciles)",
...
ggplot(lift_df, aes(x = pct_pobl, y = tasa_acum)) +
  geom_line(linewidth = 1) +
  ...
  labs(
    ...
    x = "% de población contactada (orden modelo)",
...
## 💰 Capa prescriptiva (regla de negocio)

```{r}
# Supuestos de negocio (ajústalos a tu realidad)
...
## 👤 Top 20 leads y Siguiente Mejor Acción
...
DT::datatable(
  ...
  caption = htmltools::tags$caption(style = "caption-side: top; text-align: left;",
                                    "Top 20 leads priorizados y acción sugerida")
)
```
...
4. **Siguiente Mejor Acción**: táctica recomendada por perfil/comportamiento.  
...
- La **IA en negocios** no solo predice: también **prescribe**.  
- Beneficios:
  - Mejor **priorización de leads**.
  - **Eficiencia** de presupuesto.
  - **Mayor conversión** y retorno.

**De los datos → a la acción → al impacto.**
